...
def ScoreFrameByFunction(thisFrame, nextFrame, thirdFrame, strikeFunction, spareFunction):
...
def ScoreFramesZeroToSeven(thisFrame, nextFrame, thirdFrame):
    return ScoreFrameByFunction(thisFrame, nextFrame, thirdFrame, ScoreStrikeFramesZeroToSeven, ScoreSpareFramesZeroToEight)

def ScoreFrameEight(frameEight, frameNine):
    return ScoreFrameByFunction(frameEight, frameNine, [0,0], ScoreStrikeFrameEight, ScoreSpareFramesZeroToEight)
...
def CalculateScore(frames):
    ...
    for index in range(8):
        result += ScoreFramesZeroToSeven(frames[index], frames[index + 1], frames[index + 2])
